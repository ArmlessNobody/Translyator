...
def insert():
    ...
    if flag_break == 0:
        while i < (len(list_numbers_rank)):
            ...
            if (i + 2 < len(list_numbers_rank)) and list_numbers[i] == "quatre" and list_numbers[i + 1] == "vingt" and list_numbers[i+2] == "un":
                res_number += list_numbers_rank[i].numbers * list_numbers_rank[i+1].numbers + list_numbers_rank[i+2].numbers;
                i = i + 2;
            elif (i + 2 < len(list_numbers_rank)) and list_numbers_rank[i].rank == 4 and list_numbers[i + 1] == "et" and list_numbers[i + 2] == "un":
                ...
                if i + 2 == len(list_numbers_rank):
                    ...
                    temp_text = "Разряд десятков '" + list_numbers[i+1] + "' не может стоять перед разрядом единиц '" + list_numbers[i + 2] + "'!";
                    flag_break2 = 1;
                    break;
                ...
                if list_numbers[i] == "un" and list_numbers[i + 1] == "cent":
                    ...
                    break;
                if flag_break2 == 0: flag_hundreds += 1;
                res_number += list_numbers_rank[i].numbers * list_numbers_rank[i + 1].numbers;
                i = i + 1;
            elif (i + 1 < len(list_numbers_rank)) and (list_numbers_rank[i].rank <= list_numbers_rank[i + 1].rank or (list_numbers_rank[i].rank == 2 and list_numbers_rank[i+1].rank == 1)):
                temp_text = Eror(list_numbers_rank[i], list_numbers_rank[i+1], list_numbers[i], list_numbers[i+1]);
                flag_break2 = 1;
                break;
                ...
                if list_numbers[i + 1] == 'un':
                    temp_text = "Разряд десятков '" + list_numbers[
                        i] + "' не может стоять перед разрядом единиц '" + list_numbers[i + 1] + "', вы пропустили союз 'et'!";
                    flag_break2 = 1;
                    break;
        ...
        elif flag_hundreds > 1:
            temp_text = "В введенном числе не может быть больше одного разряда сотен!";
            lbl.configure(text=temp_text);
        else:
...
window.geometry('765x165');
lbl = Label(window, text="Введите число на фрацузском языке, используя пробелы:", font='Times 15');
lbl.pack(side='top', anchor=W, ipadx=4, padx=1, ipady=3, pady=3);
f = Frame();
f.pack(side=TOP);
txt = Entry(f, width=60, font='Times 13');
txt.pack(side=LEFT, anchor=N, fill='x', ipadx=7, padx=5, ipady=8, pady=1);
insert_btn = Button(f, text="Ввод", width=25, command=insert, font='Times 13');
...
lbl2 = Label(window, text="Результат работы транслятора:", font='Times 15');
lbl2.pack(side='bottom', anchor=W, ipadx=4, padx=4, ipady=5, pady=3);
